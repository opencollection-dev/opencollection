...
describe('Tabs', () => {
  ...
  it('shows the label and its count', () => {
    const root = useRenderToDom(<Tabs tabs={tabs} testId="t" />);
    const tab = root.querySelector('[data-testid="t-tab-a"]');
    expect(tab?.text).toContain('Alpha');
    expect(tab?.querySelector('.tab-count')?.text).toBe('2');
  });

  it('activates the first tab by default and mounts only its panel', () => {
    const root = useRenderToDom(<Tabs tabs={tabs} testId="t" />);
    expect(root.querySelector('[data-testid="t-tab-a"]')?.getAttribute('aria-selected')).toBe('true');
    expect(root.querySelector('[data-testid="t-tab-b"]')?.getAttribute('aria-selected')).toBe('false');
  ...
  it('honours a controlled activeTab', () => {
    ...
    expect(root.querySelector('[data-testid="t-tab-c"]')?.getAttribute('aria-selected')).toBe('true');
  ...
  it('applies roving tabindex (only the active tab is tabbable)', () => {
    const root = useRenderToDom(<Tabs tabs={tabs} testId="t" />);
    expect(root.querySelector('[data-testid="t-tab-a"]')?.getAttribute('tabindex')).toBe('0');
    expect(root.querySelector('[data-testid="t-tab-b"]')?.getAttribute('tabindex')).toBe('-1');
  });

  it('renders the active tab’s rightElement, falling back to the shared one', () => {
    const withRight: TabItem[] = [
    ...
    expect(onA.querySelector('.tabs-right')?.text).toBe('a-right');

    const onB = useRenderToDom(<Tabs tabs={withRight} activeTab="b" rightElement={<span>shared</span>} />);
    expect(onB.querySelector('.tabs-right')?.text).toBe('shared');
  });
